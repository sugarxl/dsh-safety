...
因此本插件是**安全网**而非沙箱；要彻底治理，仍需在 DSH 上游实现安全启动与故障隔离。但"删除可恢复、崩溃可回滚"这一半能力，当前 dsh 并不具备，正是本插件补足的缺口。
...
**递归删除在任意路径无条件拒绝**是核心决策：目录递归删除的破坏半径最大、意图最难判断，采取默认拒绝、显式放行（`safe_delete`，进回收站）的策略。
...
结论：应优先配置好 DSH 自带的沙箱/审批（这是第一道、也是更强的隔离），本插件专攻 dsh 缺失的**恢复层**（回收站/快照/回滚/体检/审计）与**兜底层**（沙箱宽开时的最后防线）。
