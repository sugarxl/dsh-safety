 ...
 */
export const MAX_AUDIT_BYTES = 8 * 1024 * 1024 // 8 MB
export const MAX_AUDIT_LINES = 20000
export const MAX_ALERTS_LINES = 10000

/** If a JSONL file exceeds a size cap, keep only the most recent N lines. */
async function trimFileIfLarge(file, maxBytes, maxLines) {
  try {
...
export async function appendAudit(home, kind, data) {
  ...
  await trimFileIfLarge(auditFile(home), MAX_AUDIT_BYTES, MAX_AUDIT_LINES)
...
async function checkAlerts(home, entry) {
  ...
  if (alerts.length) {
    ...
    await trimFileIfLarge(alertsFile(home), MAX_AUDIT_BYTES, MAX_ALERTS_LINES)
  }
}
 ...
 */
/** Quote a CSV field: wrap in quotes and double any embedded quotes. */
function csvField(v) {
  const s = String(v ?? '')
