...
// ~200ms total retry window (was 5×5ms=25ms). The previous window was too
// aggressive: a slow-but-alive holder (network disk, antivirus scan, slow fsync)
// could be force-stolen mid-critical-section, breaking mutual exclusion and
// reintroducing the lost update the lock exists to prevent.
// 重试窗口 ~200ms（原 25ms 过短：慢盘/杀软扫描下活锁可能被误窃，破坏互斥）。
const LOCK_RETRIES = 10
// A stamp older than this is definitely a crashed holder — steal immediately
// instead of waiting out the whole window.
// stamp 超过此年龄必是崩溃残留，立即窃取，不必等满窗口。
...
function acquireLock(home) {
  ...
  for (;;) {
    try {
      ...
      // Held. Two ways out:
      //  1. the stamp is stale (crashed holder left it minutes ago) → steal now;
      //  2. otherwise retry until the deadline — a holder alive past the whole
      //     window is treated as stale (crashed) and force-stolen.
      // A lock with a fresh stamp is NEVER stolen early, so a slow-but-alive
      // holder keeps mutual exclusion intact.
      ...
      let stale = false
      try {
        const stamp = Number(readFileSync(path.join(lock, 'stamp'), 'utf8')) || 0
        if (stamp > 0 && Date.now() - stamp > LOCK_STALE_MS) stale = true
      } catch {
        /* no stamp yet — the holder may still be initializing; wait */
      }
      if (stale) {
        rmSync(lock, { recursive: true, force: true })
        continue
...
export function createApproval(home, { kind, target, recursive = false, what, why, consequence, alternative, systemNote, requestedBy = 'unknown' } = {}) {
  return withStateLock(home, () => {
    ...
    const req = {
      ...
      // SYSTEM-COMPUTED consequence (authoritative): filled by the caller from
      // the real path classification, so the human approving via `dsh-safety
      // allow` sees a system-backed verdict SEPARATE from the model's
      // self-reported narrative (which the system cannot verify — the model
      // could misrepresent what it is deleting or the real consequence).
      // 系统计算的后果（权威）：由调用方根据真实路径分类填充，用户在
      // `dsh-safety allow` 时看到的是系统背书的判定，与模型自述分开呈现。
