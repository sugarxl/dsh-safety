...
/**
 * Save synchronously. Used by the approval grant/consume paths, which run from
 * the synchronous tool guard where awaiting is not possible.
 */
...
export function createApproval(home, { kind, target, recursive = false, what, why, consequence, alternative, requestedBy = 'unknown' } = {}) {
  ...
  // Bound the approvals list: an agent can spam safety_ask without limit, and
  // granted/consumed/revoked records all accumulate. Drop the oldest past the
  // cap so state.json cannot grow unbounded.
  const MAX_APPROVALS = 500
  while (state.approvals.list.length > MAX_APPROVALS) state.approvals.list.shift()
