	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function Section(props) {
				// tick is read by React only to force a re-render when the locale
				// changes; its value itself is unused. (tick 仅供 locale 变化时触发重渲染，值本身未被读取)
				...
				const rpc = async (op, body) => {
					const res = await fetch("/safety/api", {
						method: "POST",
						headers: { "Content-Type": "application/json", "X-DSH-Safety": "1" },
				...
				const load = async () => {
					try {
						const res = await fetch("/safety/api", { method: "GET", signal: AbortSignal.timeout(15000) });
						if (!res.ok) throw new Error("http-" + res.status);
				...
				const btn = (label, onClick, disabled) => Button !== null
				...
				return react.createElement("div", { className: "ds-root" },
					...
					react.createElement("div", { className: "ds-toolbar" },
						btn(t("check"), runCheck, busy.check === true || state === null),
						btn(t("snapshot"), doSnapshot, busy.snapshot === true || state === null)
					),
					...
					react.createElement("ul", { className: "ds-list" },
						(state !== null && (state.trash || []).length > 0
							...
							btn(t("restore"), () => undo(e.id), busy.undo === e.id)
						))
					),
