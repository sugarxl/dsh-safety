 ...
 *
 * ZERO external imports (node builtins + `./safety-core.mjs` only) on purpose:
 * a bundle installed with a bare `link:` has no own `node_modules`, so
 * importing `@deepseek-ai/*` would fail module resolution at boot. Tool
 * definitions are hand-built (JSON Schema subset) instead of `defineTool`.
 *
 ...
 * `/safety/api` route.
 * 浏览器半区注册「安全中心」设置分区，由 /safety/api 路由支撑。
 */
...
Rules: (1) never delete with raw shell; use safe_delete. (2) run safety_check before restarting dsh after any plugin/config change. (3) run safety_snapshot before editing composition files. (4) recovery via safety_status / safety_undo / safety_restore / safety_trash.`,
  ...
  const readJson = (req) => new Promise((resolve) => {
    // Cap the request body (1 MiB) so a stray oversized POST cannot exhaust
    // memory. (请求体上限 1 MiB，防止超大 POST 耗尽内存)
