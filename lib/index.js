 ...
 *                       $HOME (the exact incident that started this plugin) is
 *                       blocked and routed to safe_delete.
 */
function buildPolicy(home, config) {
  ...
  for (const c of candidates) if (existsSync(c)) add(blockWrite, c)
...
Rules: (1) never delete with raw shell; use safe_delete. (2) run safety_check before restarting dsh after any plugin/config change. (3) run safety_snapshot before editing composition files. (4) recovery via safety_status / safety_undo / safety_restore / safety_trash.`,
  ...
  register(textTool({
    ...
    description: 'Take a last-known-good snapshot of the whole DSH composition: every profile package.json / cordis.patch.yml / cordis.yml / lockfile, home patch & settings, all plugin package.json + cordis.patch.yml, and agent-preset compositions. Run this before editing any composition/plugin file.',
    parameters: { label: { type: 'string', description: 'Optional short label, e.g. before-install.' } },
    async execute(args) {
      const manifest = await createSnapshot(home, String(args.label || ''))
      return { ok: true, text: `snapshot ${manifest.id}: ${manifest.files.length} file(s)` }
    },
  }))
