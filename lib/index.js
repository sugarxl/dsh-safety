...
function applyInner(ctx, config = {}) {
  ...
  // Memoized per (path, mtime, size): the common anti-loop case is repeated
  // denials on the SAME unchanged target, and re-enumerating a huge directory
  // synchronously on every deny would stall the event loop.
  ...
  const describeTarget = (abs) => {
    ...
    const cacheKey = `${abs}|${st.mtimeMs}|${st.size}`
    ...
    const desc = `[directory] ${names.length} top-level entr${names.length === 1 ? 'y' : 'ies'}${names.length ? ':\n  ' + top.join('\n  ') + (names.length > 20 ? `\n  … ${names.length - 20} more` : '') : ''}`
    if (describeCache.size > 100) describeCache.clear() // bound the cache
    describeCache.set(cacheKey, desc)
    return desc
  }
...
- Workflow: (1) safety_snapshot before editing composition files; (2) safety_check before restarting dsh after any plugin/config change; (3) recovery via safety_status / safety_undo / safety_restore / safety_trash.`,
  ...
  register(textTool({
    ...
    async execute(args) {
      ...
      const abs = path.resolve(String(args.path || ''))
      // The system computes the AUTHORITATIVE consequence from the real path
      // classification; the model's own what/why/consequence are unverifiable
      // self-reports, so the human sees the system-backed verdict separately.
      ...
      const req = createApproval(home, {
        ...
        systemNote: explainConsequence(cls, kind),
        requestedBy: 'agent',
      })
  ...
  if (flags.blockWrites) {
    const fsGuard = async (target, _actor, next) => {
      ...
      // Same symlink-aware classification as the tool guard (deepest existing
      // ancestor's realpath) so the waterfall cannot be bypassed by writing
      // through a symlinked parent into a protected zone.
      if (classifyWithReal(abs, policy) === 'protected') {
  ...
  /* ── fs delete waterfall (defense in depth) ────────────────────────────── */
  // Any fs-service delete tool (delete_file/remove_file/etc.) that we do not
  // know by name goes through fs/delete-intent; block protected/confirm deletes
  // there too (a granted approval is consumed, mirroring the write waterfall).
  if (flags.blockShell) {
    const fsDeleteGuard = async (target, _actor, next) => {
      const p = target && (target.displayPath || target.path)
      if (!p) return next()
      const abs = path.resolve(String(p))
      // Symlink-aware classification, consistent with the tool guard.
      const cls = classifyWithReal(abs, policy)
      if (cls === 'protected' || cls === 'confirm') {
        let isDir = false
        try { isDir = statSync(abs).isDirectory() } catch { /* missing */ }
        if (consumeApproval(home, { kind: 'delete', target: abs, recursive: isDir })) {
          guardApprovals++
          journalAppend(home, { kind: 'approval-consume', tool: 'fs/delete-intent', target: abs, cls, recursive: isDir }).catch(() => {})
          appendAudit(home, 'approval', { tool: 'fs/delete-intent', target: abs, reason: 'user-approved delete allowed' }).catch(() => {})
          return next()
        }
        await journalAppend(home, { kind: 'fs-block', target: abs, cls }).catch(() => {})
        // Fail the waterfall with a stable code; no import of @deepseek-ai/dsh-fs.
        throw Object.assign(new Error(`[dsh-safety] delete blocked on ${cls} path "${abs}" — use safe_delete (trash, undoable)`), { code: 'FS_DENIED' })
      }
      return next()
    }
    ctx.on('fs/delete-intent', fsDeleteGuard)
  }
