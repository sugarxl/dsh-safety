...
/**
 * Canonical key for path comparison: absolute + (Windows) case-folded.
 * On POSIX, Windows-style backslashes in the literal are normalized to the
 * platform separator — without this, an approval granted for a literal like
 * `C:\Temp\coop-project` could never cover `C:\Temp\coop-project\sub\build`
 * on Linux/macOS: `path.resolve` keeps `\` as a literal filename char, so
 * `isUnder`'s prefix match (`...\sub` vs `.../`) silently fails and the
 * protected/recursive classification degrades. (`resolveVariableRefs`
 * already normalized its tails; this closes the same class of bug at the
 * comparison layer, which also serves approval target matching.)
 *
 * 路径比较的规范键：绝对化 +（Windows 上）大小写折叠。POSIX 上把字面量里的
 * Windows 反斜杠归一为平台分隔符——否则在 Linux/macOS 上，为 `C:\Temp\coop-project`
 * 授予的审批永远覆盖不了 `C:\Temp\coop-project\sub\build`：path.resolve 把 `\`
 * 当成字面文件名，isUnder 前缀匹配（`...\sub` vs `.../`）静默失败，受保护区
 * 分类与递归审批匹配都会降级。
 */
export function keyOf(p) {
  const abs = path.resolve(String(p))
  return process.platform === 'win32' ? abs.toLowerCase() : abs.replace(/\\/g, path.sep)
}
 ...
 * The literal path is checked first, then the realpath of the deepest EXISTING
 * component (the target itself may not exist yet — e.g. a file about to be
 * created through a symlinked parent, or a path one level below an existing
 * symlink — so the walk climbs until it finds a component that does exist).
 * The most restrictive class wins.
 */
export function classifyWithReal(abs, policy) {
  ...
  while (probe && probe !== root && !seen.has(probe)) {
    seen.add(probe)
    try {
      ...
      /* component does not exist — keep walking up to the next ancestor */
    }
    if (existsSync(probe)) break // resolved the deepest existing component
...
const DESTRUCTIVE_VERBS = [
  ...
  /(?:^|[;\s|&])rd\b/i,
  /(?:^|[;\s|&])rmdir\b/i,
  /(?:^|[;\s|&])ri\b/i, // PowerShell alias for Remove-Item
  /\[(?:System\.)?IO\.File\]::Delete\s*\(/i, // [IO.File]::Delete (System. is optional)
  ...
  /\bgit\s+rm\b/i, // git rm removes files from the working tree
...
export function extractShellPaths(command, home) {
  ...
  const out = []
  // Both `C:\x` and `C:/x` count as absolute Windows paths — a forward-slash
  // spelling must not dodge the path classification.
  ...
  // `-Path 'C:\x'` / `--path` / `Remove-Item C:\x -Recurse`. Also capture
  // QUOTED values that contain spaces ("C:\Users\a\my file.txt"), which the
  // unquoted token pattern would truncate at the space.
...
export function isRecursiveDelete(name, args) {
  ...
  if (name === 'pwsh') {
    ...
    if (/\b(?:del|erase)\b[^\r\n]*?\s+\/\w*[sS]\w*/i.test(c)) return true // del /s, erase /s (cmd)
    return false
  }
  if (name === 'bash') {
    const c = String(a.command || '')
    ...
    if (/\b(?:rm|rmdir)(?:Sync)?\s*\([^)]*recursive\s*:\s*(?:true|1|!0)/i.test(c)) return true // rmSync(...,{recursive:true|1|!0}) — bare import form too
    return false
  }
  return false
}
...
export function destructiveTargetForCall(name, args, policy) {
  ...
  // Some editor tools also expose a `delete`/`remove` command — that is a
  // DELETE, not a write, so it must go through the same path classification
  // (protected/confirm deletes are denied and routed to safe_delete).
  if (name === 'str_replace_editor' && (String(a.command) === 'delete' || String(a.command) === 'remove')) {
    const p = a.file_path || a.path
    if (typeof p === 'string' && p.length > 0) {
      const abs = path.resolve(p)
      const cls = classifyWithReal(abs, policy)
      if (cls === 'protected' || cls === 'confirm') {
        return { action: 'deny', kind: 'delete', abs, cls, recursive: false, reason: `editor delete on ${cls} path "${abs}" — use safe_delete (trash, undoable)` }
      }
    }
  }
  if (name === 'pwsh' || name === 'bash') {
    ...
    const paths = extractShellPaths(command, policy?.home)
    ...
    // Carry the first explicit target path (if any) so a cooperative-mode
    // approval granted via `dsh-safety allow --path <dir> --recursive` can
    // match it — a target-less `abs: null` would only ever match the fully
    // generic (target-null) approval.
    if (recursive) {
      const first = paths[0] ? path.resolve(paths[0]) : null
      return { action: 'deny', kind: 'delete', abs: first, cls: 'recursive', recursive: true, reason: 'recursive directory delete — use safe_delete (moves to trash, undoable)' }
    }
    return { action: 'allow' }
  }
  if (name === 'run_code') {
    ...
    // Writes too: arbitrary code can `fs.writeFileSync(protectedPath, ...)` and
    // dodge the write/edit tool guard entirely, so scan for write verbs as well.
    const WRITE_VERB = /\b(?:writeFile|appendFile|rename|copyFile|cp|truncate|createWriteStream|writeFileSync|appendFileSync|renameSync|copyFileSync|cpSync|truncateSync)\s*\(/i
    const isWriteCode = code.length > 0 && WRITE_VERB.test(code)
    if (!isDestructiveCode && !isWriteCode) return { action: 'allow' }
    const recursive = isRecursiveDelete('bash', { command: code }) || /\b(?:rm|rmdir)(?:Sync)?\s*\([^)]*recursive\s*:\s*(?:true|1|!0)/i.test(code)
    // Explicit absolute paths in the code body get the SAME classification as
    // shell commands: deletes on protected/confirm paths and WRITES on
    // protected paths are denied, not silently allowed.
    for (const p of extractShellPaths(code, policy?.home)) {
      const abs = path.resolve(p)
      const cls = classifyWithReal(abs, policy)
      if (cls === 'protected') {
        return { action: 'deny', kind: isWriteCode ? 'write' : 'delete', abs, cls, recursive, reason: `run_code ${isWriteCode ? 'write' : 'delete'} on protected path "${abs}"` }
      }
      if (cls === 'confirm' && isDestructiveCode) {
    ...
    // Marker fallback stays for DELETES (any protected/confirm marker). For
    // WRITES we rely on the precise path classification above — a marker
    // fallback would over-block legitimate edits (every path under the DSH home
    // contains ".dsh", and confirm-zone edits are allowed by policy).
    if (isDestructiveCode) {
      for (const marker of markersFor(policy)) {
        if (lower.includes(marker)) {
    ...
    if (recursive && isDestructiveCode) {
...
export function resolveVariableRefs(command, policy = {}) {
  ...
  const push = (name, tail) => {
    ...
    // Normalize Windows-style backslashes to the platform separator. Without
    // this, on POSIX a tail like `\profiles\web\x.js` stays a single filename
    // component containing literal backslashes, so isUnder/classify prefix
    // matching fails and a real protected path is classified as free.
    // (On Windows `\\` already IS the separator, so this is a no-op.)
    const cleanTail = String(tail).replace(/^[\\/]+/, '').replace(/\\/g, path.sep)
    out.push(path.join(base, cleanTail))
  }
...
export function scanPatchIds(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]
    // Quoted ids and trailing inline comments (`- id: foo # comment`) must not
    // break the scan — previously a comment made the `$` anchor fail and the
    // row was silently skipped, hiding duplicate-row problems.
    const m = /^\s*(?:-\s+)?id\s*:\s*(?:"([^"#\s]+)"|'([^'#\s]+)'|([^"'#\s]+))\s*(?:#.*)?$/.exec(line)
    if (m) ids.push({ id: m[1] || m[2] || m[3], line: i + 1 })
  }
  return ids
}
...
export const MAX_JOURNAL_BYTES = 8 * 1024 * 1024 // 8 MB
export const MAX_JOURNAL_LINES = 20000
...
export async function journalAppend(home, entry) {
  ...
  // Bound the journal: once it exceeds MAX_JOURNAL_BYTES keep only the most
  // recent MAX_JOURNAL_LINES. The check is a cheap stat; the rewrite only
  // triggers when the cap is crossed (append-only, so this is rare).
  try {
    const st = statSync(file)
    if (st.size > MAX_JOURNAL_BYTES) {
      const lines = (await fsp.readFile(file, 'utf8')).split(/\r?\n/).filter((l) => l.trim() !== '')
      await fsp.writeFile(file, lines.slice(-MAX_JOURNAL_LINES).join('\n') + '\n', 'utf8')
    }
  } catch {
    /* best effort */
  }
...
export async function restoreSnapshot(home, id) {
  ...
  // Random suffix so two restores in the same second never share a backup dir
  // (which would collide/mix the backups of two separate rollbacks).
  ...
  try {
    for (const item of plan) {
      ...
      // Integrity gate (FAIL-CLOSED): every manifest entry must carry a
      // checksum — createSnapshot always writes one, so a missing sha256 means
      // the manifest was tampered with or the snapshot is corrupt. Only then is
      // the copy verified against the manifest, so corruption can never
      // propagate onto the live composition. (Previously a deleted sha256 field
      // silently skipped the check.)
      const entry = byRel.get(item.rel)
      if (!entry || !entry.sha256) {
        throw new Error(`snapshot manifest entry for "${item.rel}" is missing its checksum (tampered or corrupt snapshot)`)
      }
      const actual = await sha256File(item.src)
      if (actual !== entry.sha256) {
        throw new Error(`snapshot checksum mismatch for "${item.rel}" (corrupt snapshot)`)
      }
...
async function sha256File(file) {
  ...
  try {
    // Stream in chunks so large files (lockfiles etc.) are not loaded into
    // memory whole.
