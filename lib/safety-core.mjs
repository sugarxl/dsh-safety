...
export function isRecursiveDelete(name, args) {
  ...
  if (name === 'bash') {
    ...
    if (/\.(?:rm|rmdir)(?:Sync)?\s*\([^)]*recursive\s*:\s*true/i.test(c)) return true // fs.rmSync(...,{recursive:true}) etc.
    return false
  }
  return false
}
...
export function destructiveTargetForCall(name, args, policy) {
  ...
  if (name === 'run_code') {
    ...
    if (typeof code !== 'string' || code.length === 0 || !hasDestructiveVerb(code)) return { action: 'allow' }
    const recursive = isRecursiveDelete('bash', { command: code })
...
export async function restoreSnapshot(home, id) {
  ...
  for (const entry of manifest.files || []) {
    ...
    plan.push({ rel: entry.rel, dest, src })
  }
