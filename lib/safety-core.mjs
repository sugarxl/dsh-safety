...
export async function restoreSnapshot(home, id) {
  ...
  try {
    for (const item of plan) {
      ...
      // Integrity gate: never restore a file whose content has drifted from the
      // manifest — a corrupt/tampered snapshot must not propagate corruption
      // onto the live composition (that is what restore is meant to fix).
